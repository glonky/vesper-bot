...
import { NonRetriableError, RetriableError, ExtendedError, NotFoundError, RetryError } from '../errors';
 ...
 * It will automatically bail if a Shuffl NonRetriableError was thrown.
 *
...
export async function retry<T>(fn: RetryFunction<T>, props?: RetryProps): Promise<T> {
  ...
  const { factor, maxTimeout, minTimeout, randomize, retries, forever, maxRetryTime } = config.retry;
  ...
  bailOnErrors.push(NonRetriableError);
  ...
  try {
    return await asyncRetry(async (bail, retryAttempt) => {
      ...
      return new Promise<T>(async (resolve, reject) => {
        try {
          ...
          if (shouldBailForError || shouldBailForOriginalError) {
          ...
          return reject(new RetriableError('Retry failed, throwing error', { error }));
        }
      });
    }, overrides);
  } catch (err) {
    ...
    if (err instanceof RetryError) {
      ...
      if (originalError instanceof NotFoundError) {
        ...
        logger.error('Failed and bailed early because we found RetryError, re-throwing actual error.', {
          error: originalError,
          timeElapsedMs,
          totalRetryAttempts,
        });
